...
Requirements: Windows (32 or 64 bit), Python 32-bit
...
class Camera:
    ...
    def openCamera(self, cid=None):  # attempt initial connection to camera
        ...
        if self.h == -1:
            raise TypeError(f"Camera not found on open attempt with {get_dll_path()}")
        else:
            self.isopen = True
    ...
    def getGain(self) -> dict[str, int]:
        ...
        self.openCamera()
        rc = self.dll.CxGetGain(self.h, ct.byref(gg1), ct.byref(gr), ct.byref(gg2), ct.byref(gb))
        self.closeCamera()
        if rc == 0:
    ...
    def setAllGain(self, gainreq: int) -> dict[str, int]:
        # sets gain for all colors simultaneously
        if gainreq is None:
            return None
    ...
    def grabFrame(self):  # grab latest frame in stream
        ...
        return np.asarray(imbuffer).reshape((self.ypix, self.xpix), order="C")
...
class _TCameraInfoEx(ct.Structure):
    _fields_ = [("HWModelID", ct.c_ushort), ("HWVersion", ct.c_ushort), ("HWSerial", ct.c_ulong)]
...
class Convert:
    ...
    def BayerToRgb(self, bayerimg: np.ndarray, bayerint: int):
        ...
        assert bayerint in range(6), (
            "bayer mode must be in 0,1,2,3,4,5\n "
        ...
        rc = self.dll.CxBayerToRgb(
            ct.byref(inbuffer), Width, Height, BayerAlg, ct.byref(outbuffer)
        )
        if rc == 0:
        ...
        dimg = np.asarray(outbuffer).reshape((h, w, 3), order="C")
